...
run_hs <- function(
  ...
  # Fit horseshoe-prior reference model
  ...
  patient_prob_draws <- list(
    ...
    ref_hs = make_patient_draws(ref_predictions$p_draws, selected_ids, length(yval)),
    hs_1se = make_patient_draws(suggest_predictions$p_draws, selected_ids, length(yval)),
    hs_best = make_patient_draws(best_predictions$p_draws, selected_ids, length(yval))
  )
